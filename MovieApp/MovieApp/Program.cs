...
namespace MovieApp
{
    class Program
    {
        ...
        public static async void GetBooking()
        {
            //Define your baseUrl
            //string baseUrl = "http://pokeapi.co/api/v2/pokemon/";
            string baseUrl = "http://localhost:5000/api/booking/"; 
                    ...
                    using (HttpResponseMessage res = await client.GetAsync(baseURL))
                    {
                        ...
                        using (HttpContent content = res.Content)
                        {
                            ...
                            if (data != null)
                            {
                                ...
                                MovieItem movieItem = new MovieItem(name: $"{dataObj["name"]}");
                                //Log your pokeItem's name to the Console.
                                Console.WriteLine("Booking Name: {0}", movieItem.Name);
                            }
                            else
                            {
